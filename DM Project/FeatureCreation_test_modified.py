import csv
final =csv.writer(open('test_edit3.csv','wb+'))
training= csv.reader(open("test.csv","rb"), delimiter=',')
###Set Column Names
for row in training:
	final.writerow(["RefID"]+["IsBadBuy"]+["VehicleAge"]+["WheelType"]+["DifAquiredAvg_CurAvgAuction"]+["DifAquiredAboveAvg_CurAboveAvgRetail"]+["DifAquiredAvg_CurAvgRetail"]+["DifAquiredAboveAvg_CurAboveAvgAuction"]+["AdjVehOd"])
	break
...
for row in training:
	...
	if row[22]=="0" or row[22]=="NULL":
		final.writerow([row0]+[""]+[row4]+[row12])
		continue
...
### Veh Miles divided by age
	...
	final.writerow([row0]+[""]+[row4]+[row12]+[dif1]+[dif2]+[dif3]+[dif4]+[dif5])
